function [spd_data, ratio_box_mean, ratio_box_std, ratio_line_mean, ratio_line_std] = spindle_mat_average_ratio(spd_data, list_good, NUM_BIN)
...
for i = 1:length(list_good);
    ...
    if isfield(spd_data{idx}, 'ratio_box_100') && isfield(spd_data{idx}, 'ratio_line_100');
        ratio_box_all(i, :) = spd_data{idx}.ratio_box_100;
        ratio_line_all(i, :) = spd_data{idx}.ratio_line_100;
    else
